...
func TestPostEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PRANK_SHOW_TEST_POST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func postBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PRANK_SHOW_TEST_POST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PRANK_SHOW_TEST_POST_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["PRANK_SHOW_TEST_LIVE"] == "TRUE" {
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["PRANK_SHOW_TEST_EXPLAIN"] == "TRUE",
		live:          live,
