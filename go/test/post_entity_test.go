...
func TestPostEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PRANKSHOW_TEST_POST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func postBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PRANKSHOW_TEST_POST_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"PRANKSHOW_TEST_POST_ENTID": idmap,
		"PRANKSHOW_TEST_LIVE":      "FALSE",
		"PRANKSHOW_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["PRANKSHOW_TEST_POST_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["PRANKSHOW_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
			},
			extra,
		})
		client = sdk.NewPrankShowSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["PRANKSHOW_TEST_EXPLAIN"] == "TRUE",
		live:          live,
