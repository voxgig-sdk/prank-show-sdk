...
import (
	...
	omni "github.com/voxgig-sdk/prank-show-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PrankShowError); is {
				out := map[string]any{
					"name":    "PrankShowError",
...
func omniCtx(arg any, client *sdk.PrankShowSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PrankShowSDK); is {
			client = live
