...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.PRANKSHOW_TEST_LIVE ||
    'TRUE' === process.env.PRANKSHOW_TEST_OVERRIDE
  ) {
  ...
  m.PRANKSHOW_TEST_EXPLAIN = process.env.PRANKSHOW_TEST_EXPLAIN || m.PRANKSHOW_TEST_EXPLAIN
