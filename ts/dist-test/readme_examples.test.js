...
function isRunnable(code) {
    return (/new\s+PrankShowSDK\b/.test(code) ||
        /\bPrankShowSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PrankShowSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PrankShowSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
