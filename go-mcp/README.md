...
[MCP](https://modelcontextprotocol.io) server exposing the PrankShow SDK as
two agent tools — `prank-show_list` and `prank-show_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/prank-show-mcp)
make build

# 2. Provide credentials via the environment
export PRANK_SHOW_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user prank-show \
  -- /absolute/path/to/prank-show-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./prank-show-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// prank-show_list: first page of records
{ "entity": "post" }
{ "entity": "post", "query": { } }

// prank-show_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/prank-show-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user prank-show \
     -- "$PWD"/dist/*/prank-show-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `prank-show_list` and `prank-show_load` tools now appear
   in new sessions. Ask the agent to *"list post using prank-show"*
   and it calls `prank-show_list` with `{"entity":"post"}`.
...
```sh
export PRANK_SHOW_APIKEY=sk_live_xxx            # API key
export PRANK_SHOW_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `prank-show_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `prank-show_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PRANK_SHOW_APIKEY` | API key sent with every request. |
| `PRANK_SHOW_BASE` | Optional override of the API base URL. |
...
```sh
./prank-show-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"prank-show_load","arguments":{"entity":"post","query":{"id":1}}}}'
```
