...
// PrankShow SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PrankShowOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PrankShowOmni::structfix($now, Struct::undef()))
                        !== json_encode(PrankShowOmni::structfix($val, Struct::undef()))) {
