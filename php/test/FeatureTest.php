...
class FtCtrl extends PrankShowControl
{
...
class FtClient
{
    ...
    private PrankShowUtility $utility;

    public function __construct(PrankShowUtility $utility, array $options)
    {
    ...
    public function get_utility(): PrankShowUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PrankShowContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PrankShowContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PrankShowContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PrankShowConfig::make_feature($name);
    ...
    private static function buildUrl(PrankShowSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PrankShowError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PrankShowResponse($fetched);
        }
    ...
    private function fail(PrankShowContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PrankShowContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
