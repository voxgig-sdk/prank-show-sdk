...
class PostEntity
{
    ...
    {
        if ($args) {
            $this->_data = PrankShowHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = PrankShowHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws PrankShowError on failure (item-5 convention).
     */
