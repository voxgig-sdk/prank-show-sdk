...
require_relative 'PrankShow_types'
...
class PrankShowSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PrankShowHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PrankShowHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PrankShowFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PrankShowUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PrankShowHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PrankShowHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PrankShowHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PrankShowError.new(
        "#{op}_allow",
        "PrankShowSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PrankShowHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PrankShowError.new(
        "graphql_error", "PrankShowSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
