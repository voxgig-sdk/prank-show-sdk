...
The Ruby SDK for the PrankShow API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "PrankShow_sdk"
client = PrankShowSDK.new(options)
```
...
```ruby
client = PrankShowSDK.test(testopts, sdkopts)
```
...
raise a `PrankShowError` (a `StandardError` subclass), so wrap
...
The main module (`PrankShow_sdk`) exports the SDK class
