...
class PostEntity
  ...
  def data_set(args)
    if args
      @_data = PrankShowHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PrankShowHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Post>, Array] the matching Post items; raises PrankShowError on failure
