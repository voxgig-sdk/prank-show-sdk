...
class TestPostEntity:
    ...
    def test_should_stream(self):
        ...
        base = PrankShowSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PRANK_SHOW_TEST_POST_ENTID JSON to run live")
        client = setup["client"]
...
def _post_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PRANK_SHOW_TEST_POST_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("PRANK_SHOW_TEST_LIVE") == "TRUE":
        ...
        client = PrankShowSDK(helpers.to_map(merged_opts))

    _live = env.get("PRANK_SHOW_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("PRANK_SHOW_TEST_EXPLAIN") == "TRUE",
        "live": _live,
