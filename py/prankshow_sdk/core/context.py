...
from prankshow_sdk.core.control import PrankShowControl
...
from prankshow_sdk.core.helpers import get_ctx_prop, to_map
...
class PrankShowContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, PrankShowSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PrankShowResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PrankShowResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return PrankShowError(code, msg, self)
