...
boru-driven command-line client **and** interactive REPL for the PrankShow
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/prank-show-cli)
make build
...
./prank-show-cli list post

# 5. Override the API base URL for a single call
PRANK_SHOW_BASE=https://api.example.com ./prank-show-cli list post

# 6. No arguments -> interactive REPL
./prank-show-cli
prank-show> list post
prank-show> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/prank-show-cli
   ```
...
Run with no arguments to open a REPL (prompt `prank-show>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/prank-show-cli`. |
...
(`./prank-show-cli <expr>`) and interactively (the REPL), and expressions compose the
