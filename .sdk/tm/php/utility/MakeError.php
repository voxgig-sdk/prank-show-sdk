...
class PrankShowMakeError
{
    public static function call(?PrankShowContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PrankShowContext([], null);
        }
        $op = $ctx->op ?? new PrankShowOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PrankShowResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PrankShowError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PrankShowError('', $msg, $ctx);
        ...
        if ($err instanceof PrankShowError) {
