...
{
    private static function utility(): PrankShowUtility
    {
        return new PrankShowUtility();
    }
    ...
    private static function ctx(array $over = []): PrankShowContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PrankShowContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PrankShowOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PrankShowError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PrankShowOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PrankShowMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PrankShowOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PrankShowMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PrankShowOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PrankShowMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PrankShowSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PrankShowMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PrankShowResponse([]),
            'result' => new PrankShowResult([]),
        ]);
        [, $err] = PrankShowMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PrankShowSpec([]),
            'response' => null,
            'result' => new PrankShowResult([]),
        ]);
        [, $err] = PrankShowMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PrankShowSpec(['step' => 's']),
            'response' => new PrankShowResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PrankShowResult([]),
        ]);
        [, $err] = PrankShowMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PrankShowSpec(['step' => 's']),
            'response' => new PrankShowResponse(self::resp(200, ['v' => 1])),
            'result' => new PrankShowResult([]),
        ]);
        [, $err] = PrankShowMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PrankShowSpec(['step' => 's']),
            'response' => new PrankShowResponse(self::resp(200, ['v' => 2])),
            'result' => new PrankShowResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PrankShowMakeResponse::call($ctx);
    ...
    {
        $preset = new PrankShowResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PrankShowSpec([]),
            'response' => new PrankShowResponse([]),
            'result' => new PrankShowResult([]),
        ...
        [$got, $err] = PrankShowMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PrankShowResult([])]);
        [, $err] = PrankShowMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PrankShowSpec([]), 'result' => null]);
        [, $err] = PrankShowMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PrankShowSpec(['step' => 's']),
            'result' => new PrankShowResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PrankShowOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PrankShowMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PrankShowSpec(['step' => 's']),
            'result' => new PrankShowResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PrankShowOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PrankShowMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PrankShowResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PrankShowSpec([]),
            'result' => new PrankShowResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PrankShowMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PrankShowMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PrankShowError('boom', 'boom');
        $utility->fetcher = function (PrankShowContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PrankShowSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PrankShowMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PrankShowContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PrankShowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PrankShowResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PrankShowContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PrankShowSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PrankShowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PrankShowResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PrankShowSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PrankShowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PrankShowMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PrankShowMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PrankShowResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PrankShowDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PrankShowResult(['ok' => false])]);
        $this->expectException(PrankShowError::class);
        PrankShowDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PrankShowResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PrankShowMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PrankShowMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PrankShowBaseFeature();
        $b = new PrankShowBaseFeature();
        PrankShowFeatureAdd::call($ctx, $a);
        PrankShowFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PrankShowBaseFeature
    {
        $f = new PrankShowBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PrankShowFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?PrankShowSpec $spec): PrankShowContext
    {
    ...
    private static function auth_bags(): PrankShowSpec
    {
        return new PrankShowSpec(['headers' => [], 'query' => []]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PrankShowPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PrankShowResponse(['headers' => null]),
            'result' => new PrankShowResult([]),
        ]);
        PrankShowResultHeaders::call($ctx);
