...
module PrankShowUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PrankShowResponse.new({})
    ...
    if fetch_err
      ...
      response = PrankShowResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PrankShowResponse.new(fetched)
    else
